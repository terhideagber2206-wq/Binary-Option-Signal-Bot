...
// Telegram bot (polling is perfect for Railway)
...
async function fetchCandles(symbol, interval, limit) {
  ...
  if (!data  !data.values  !Array.isArray(data.values)) throw new Error("No data");
...
function getSignal({ close, high, low, time }) {
  ...
  let side = null;
  if (e9_prev < e21_prev && e9_now > e21_now && rsi_now <= RSI_HIGH) side = "CALL";
